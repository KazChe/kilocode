...
| 6 | (this commit) `feat(telemetry): add CausalityCarrier module + withBaggage helper` | New `packages/kilo-telemetry/src/causality-carrier.ts` (pure OTel) and new `packages/opencode/src/effect/otel-baggage.ts` (Effect-aware helper with `<A, E>` signature; R = never. Tried `<A, E, R>` via `Effect.callback`'s native generic but that does not propagate AsyncLocalStorage at the resume site). Refactor `packages/opencode/test/effect/otel-baggage-propagation.test.ts` to import `withBaggage` from production module. Tests for `CausalityCarrier`. Plan updated to record the helper module split and the R-limitation discovery. | ⏳ this commit | No (not wired yet) |
